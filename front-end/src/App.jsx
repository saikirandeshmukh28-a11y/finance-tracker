      ...
      <div style={{flex:"0 0 48%",background:"linear-gradient(145deg,#1e40af 0%,#2563eb 45%,#1d4ed8 100%)",display:"flex",flexDirection:"column",justifyContent:"space-between",padding:"48px 56px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,position:"relative"}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:"rgba(255,255,255,0.65)",fontWeight:500}}>Personal Finance</div>
          </div>
        </div>
        <div style={{position:"relative"}}>
          <div style={{fontSize:36,fontWeight:800,color:"white",lineHeight:1.25,letterSpacing:"-0.8px",marginBottom:16}}>Take control of your finances</div>
          <div style={{fontSize:15,color:"rgba(255,255,255,0.75)",lineHeight:1.7,marginBottom:36}}>Track expenses, manage budgets, and reach your financial goals.</div>
          ...
          {tab==="login"?(
            <>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                {[{label:"Google",bg:"#fff",border:"var(--border)",img:"https://www.google.com/favicon.ico"},{label:"Apple",bg:"#000",border:"#000",textColor:"white"}].map(o=>(
                  <button key={o.label} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,padding:"10px",background:o.bg,border:`1px solid ${o.border}`,borderRadius:9,cursor:"pointer",fontSize:13,fontWeight:600,color:o.textColor||"var(--text)"}}>
            ...
            <>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
                <div>
                  ...
                  <div className="auth-input-wrap"><User size={14} className="auth-icon"/><input className="ft-input" placeholder="Arjun Kumar" value={regForm.name} onChange={e=>setRegForm({...regForm,name:e.target.value})}/></div>
                </div>
                <div>
                  <label style={{display:"block",fontSize:12.5,fontWeight:600,color:"var(--text2)",marginBottom:6}}>Phone</label>
                  <div className="auth-input-wrap"><Phone size={14} className="auth-icon"/><input className="ft-input" placeholder="+91 9876543210" value={regForm.phone} onChange={e=>setRegForm({...regForm,phone:e.target.value})}/></div>
                </div>
              </div>
              <div style={{marginBottom:14}}>
                <label style={{display:"block",fontSize:12.5,fontWeight:600,color:"var(--text2)",marginBottom:6}}>Email address *</label>
                <div className="auth-input-wrap"><Mail size={14} className="auth-icon"/><input className="ft-input" type="email" placeholder="you@example.com" value={regForm.email} onChange={e=>setRegForm({...regForm,email:e.target.value})}/></div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
                <div>
                  <label style={{display:"block",fontSize:12.5,fontWeight:600,color:"var(--text2)",marginBottom:6}}>Password *</label>
                  <div className="auth-input-wrap"><Lock size={14} className="auth-icon"/><input className="ft-input" type={showPass?"text":"password"} placeholder="Min. 8 characters" value={regForm.password} onChange={e=>setRegForm({...regForm,password:e.target.value})}/><button className="eye-btn" onClick={()=>setShowPass(!showPass)}>{showPass?<EyeOff size={14}/>:<Eye size={14}/>}</button></div>
                </div>
                <div>
                  <label style={{display:"block",fontSize:12.5,fontWeight:600,color:"var(--text2)",marginBottom:6}}>Confirm *</label>
                  <div className="auth-input-wrap"><Lock size={14} className="auth-icon"/><input className="ft-input" type={showPass2?"text":"password"} placeholder="Repeat password" value={regForm.confirm} onChange={e=>setRegForm({...regForm,confirm:e.target.value})}/><button className="eye-btn" onClick={()=>setShowPass2(!showPass2)}>{showPass2?<EyeOff size={14}/>:<Eye size={14}/>}</button></div>
                </div>
              </div>
              {regForm.password&&(
                <div style={{marginBottom:18}}>
                  <div style={{display:"flex",gap:4,marginBottom:5}}>
                    {[1,2,3,4].map(lvl=>{
                      const s=regForm.password.length>=12&&/[A-Z]/.test(regForm.password)&&/[0-9]/.test(regForm.password)?4:regForm.password.length>=10?3:regForm.password.length>=8?2:1;
                      return <div key={lvl} style={{flex:1,height:4,borderRadius:99,background:lvl<=s?(s>=4?"var(--green)":s>=3?"var(--amber)":"var(--red)"):"var(--border)"}}/>;
                    })}
                  </div>
                  <div style={{fontSize:11.5,color:"var(--muted)"}}>Strength: {regForm.password.length<8?"Weak":regForm.password.length<10?"Fair":regForm.password.length<12?"Good":"Strong"}</div>
                </div>
              )}
              <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:20}}>
                <input type="checkbox" id="terms" style={{marginTop:2,accentColor:"var(--blue)",cursor:"pointer"}}/>
                <label htmlFor="terms" style={{fontSize:12.5,color:"var(--muted)",cursor:"pointer",lineHeight:1.5}}>I agree to the <span style={{color:"var(--blue)",fontWeight:600}}>Terms of Service</span> and <span style={{color:"var(--blue)",fontWeight:600}}>Privacy Policy</span></label>
              </div>
...
function Dashboard({user,onLogout}) {
  ...
  const askAI=()=>{
    ...
    setTimeout(()=>{
      ...
      if(q.includes("food")||q.includes("dining"))
        ans=`You've spent ${fmt(catBreakdown.find(c=>c[0]==="food")?.[1]||0)} on Food this month. Consider meal prepping to cut 15-20%.`;
      else if(q.includes("save")||q.includes("saving"))
        ans=`Net savings: ${fmt(savings)} — ${Math.round(savings/Math.max(mtIncome,1)*100)}% rate. ${savings/Math.max(mtIncome,1)>0.2?"Above 20% target!":"Below 20% — try reducing shopping."}`;
      else if(q.includes("budget"))
        ans=`${budgets.filter(b=>b.spent/b.limit>0.9).length} budgets need attention. Pause discretionary spending.`;
      else if(q.includes("net worth"))
        ans=`Net worth: ${fmt(netWorth)} — across all linked accounts.`;
      else
        ans=`Income: ${fmt(mtIncome)} · Expenses: ${fmt(mtExpense)} · Savings: ${fmt(savings)} (${Math.round(savings/Math.max(mtIncome,1)*100)}%). Spending ${mtExpense>lmExpense?"↑ up":"↓ down"} ${fmt(Math.abs(mtExpense-lmExpense))} vs last month.`;
  ...
  const addTxn=async(form)=>{
    try {
      ...
      setTxns([res.data.data,...txns]);
      setModal(null);
      showToast("Transaction added.");
    } catch(err){showToast("Failed to add transaction.","error");}
  };

  const delTxn=async(id)=>{
    try {
      await deleteTransaction(id);
      setTxns(txns.filter(t=>(t._id||t.id)!==id));
        ...
        <div style={{display:"flex",alignItems:"center",gap:13}}>
          ...
          <div>
            ...
            <div style={{fontSize:12,color:"var(--muted)",marginTop:2,display:"flex",alignItems:"center",gap:6}}>
              ...
              <span className="cat-pill" style={{background:Cat?.bg,color:Cat?.color,border:`1px solid ${Cat?.bdr}`}}>{Cat?.label}</span>
            </div>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:14,fontWeight:700,color:t.type==="credit"?"var(--green)":"var(--text)",fontFamily:"'Manrope',sans-serif",display:"flex",alignItems:"center",gap:4}}>
              {t.type==="credit"?<ArrowDownLeft size={14} color="var(--green)"/>:<ArrowUpRight size={14} color="var(--red)"/>}
              {t.type==="credit"?"+":"-"}{fmt(t.amount,2)}
            </div>
          </div>
          {showDelete&&<button className="btn-danger" onClick={()=>delTxn(txnId)} style={{padding:"5px 8px"}}><Trash2 size={12}/></button>}
        </div>
      </div>
    );
  };
    ...
    <div className="page-fade">
      ...
      {dataLoading?(
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:200,gap:12,color:"var(--muted)"}}>
          ...
          <span style={{fontSize:14,fontWeight:500}}>Loading your data…</span>
        </div>
      ):(
        <>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:22}}>
            {[
              ...
              {label:"Net Savings",value:fmt(savings),sub:`${Math.round(savings/Math.max(mtIncome,1)*100)}% savings rate`,Icon:PiggyBank,color:"var(--purple)",bg:"var(--purple-bg)"},
            ].map((c,i)=>(
              <div key={i} className="stat-card">
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                  ...
                  <div style={{width:34,height:34,borderRadius:9,background:c.bg,display:"flex",alignItems:"center",justifyContent:"center"}}><c.Icon size={16} color={c.color}/></div>
                </div>
                ...
                {ACCOUNTS.map(a=>(
                  <div key={a.id} className="acct-row">
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <div style={{width:36,height:36,borderRadius:9,background:a.bg,display:"flex",alignItems:"center",justifyContent:"center",border:`1px solid ${a.color}20`}}><a.Icon size={16} color={a.color}/></div>
                      <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:18}}>
            <div className="card" style={{padding:22}}>
              ...
              {budgets.map(b=>{
                ...
                return (
                  <div key={b.category} style={{marginBottom:15}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                      <div style={{display:"flex",alignItems:"center",gap:7}}>
                        {C?.Icon&&<C.Icon size={13} color={C.color}/>}
                        <span style={{fontSize:13,fontWeight:600}}>{C?.label}</span>
                      </div>
                      <span style={{fontSize:12,color:"var(--muted)",fontFamily:"'Manrope',sans-serif"}}>{fmt(b.spent)} / {fmt(b.limit)}</span>
                    </div>
            ...
            <div className="card" style={{padding:22}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
                <div style={{fontSize:15,fontWeight:700,display:"flex",alignItems:"center",gap:8}}><Target size={15} color="var(--blue)"/>Savings Goals</div>
                <button className="btn-secondary" style={{fontSize:12,padding:"5px 12px"}} onClick={()=>setPage("goals")}>Manage</button>
              </div>
              {goals.slice(0,4).map(g=>{
                const pct=clamp((g.saved/g.target)*100,0,100);
                const GIcon=g.Icon||Target;
                return (
                  <div key={g._id||g.id} style={{marginBottom:15}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                      <div style={{display:"flex",alignItems:"center",gap:7}}><GIcon size={13} color={g.color}/><span style={{fontSize:13,fontWeight:600}}>{g.name}</span></div>
                      <span style={{fontSize:12,color:"var(--muted)"}}>{Math.round(pct)}%</span>
                    </div>
          ...
          <div className="card" style={{padding:22,marginBottom:18,background:"linear-gradient(135deg,#f0f7ff,#f5f3ff)"}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
              <div style={{width:42,height:42,borderRadius:10,background:"var(--blue)",display:"flex",alignItems:"center",justifyContent:"center"}}><Sparkles size={19} color="white"/></div>
              <div>
            ...
            <div style={{display:"flex",gap:10,marginBottom:12}}>
              <div style={{position:"relative",flex:1}}>
                ...
                <input className="ft-input" style={{paddingLeft:34,background:"white"}} placeholder='Try "How much did I spend on food?" or "What is my savings rate?"' value={aiQuery} onChange={e=>setAiQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&askAI()}/>
              </div>
          ...
          <div className="card" style={{overflow:"hidden"}}>
            ...
            {txns.slice(0,8).map(t=><TxnRow key={t._id||t.id} t={t}/>)}
            {txns.length===0&&<div style={{padding:"32px",textAlign:"center",color:"var(--muted)",fontSize:13}}>No transactions yet. Add your first one!</div>}
          </div>
        </>
      )}
    </div>
    ...
    return (
      <div className="page-fade">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
          ...
          <button className="btn-primary" onClick={()=>{setBudgets(lb);showToast("Budgets saved.");}}><CheckCircle2 size={15}/>Save Changes</button>
        </div>
          ...
          <div style={{display:"flex",flexWrap:"wrap"}}>
            {[
              ...
              <div key={s.label} style={{flex:1,minWidth:120,padding:"4px 20px",borderRight:i<3?"1px solid var(--blue-bdr)":"none"}}>
                <div style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:"var(--muted)",fontWeight:500,marginBottom:4}}><s.Icon size={12} color="var(--blue)"/>{s.label}</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:16}}>
          {lb.map((b,i)=>{
            ...
            const C=CATEGORIES[b.category];
            return (
              <div key={b.category||i} className="card" style={{padding:22}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
                  <div style={{display:"flex",alignItems:"center",gap:11}}>
                    <div style={{width:42,height:42,borderRadius:10,background:C?.bg,border:`1px solid ${C?.bdr}`,display:"flex",alignItems:"center",justifyContent:"center"}}>
                ...
                <div className="progress-track" style={{height:8,marginBottom:12}}><div className="progress-fill" style={{width:`${pct}%`,background:barColor,height:"100%"}}/></div>
      ...
      <div className="page-fade">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <button className="btn-primary" onClick={()=>{setGoals(lg);showToast("Goals saved.");}}><CheckCircle2 size={15}/>Save</button>
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:16}}>
          {lg.map((g,i)=>{
            ...
            return (
              <div key={gid} className="card" style={{padding:24,borderTop:`3px solid ${g.color}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:18}}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    <div style={{width:48,height:48,borderRadius:12,background:`${g.color}12`,display:"flex",alignItems:"center",justifyContent:"center",border:`1px solid ${g.color}25`}}><GIcon size={22} color={g.color}/></div>
                    <div>
                      <div style={{fontSize:16,fontWeight:700}}>{g.name}</div>
                      <div style={{fontSize:12,color:"var(--muted)",marginTop:2,display:"flex",alignItems:"center",gap:5}}><Calendar size={11}/>{daysLeft>0?`${daysLeft} days left`:"Past deadline"} · {fmtDate(g.deadline)}</div>
                    </div>
                  </div>
                  <div style={{textAlign:"right"}}>
                ...
                <div className="progress-track" style={{height:10,marginBottom:10}}><div className="progress-fill" style={{width:`${pct}%`,background:g.color,height:"100%"}}/></div>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:14}}>
                ...
                {monthly>0&&(
                  <div style={{padding:"10px 14px",borderRadius:9,background:`${g.color}10`,border:`1px solid ${g.color}20`,fontSize:13,marginBottom:14,color:"var(--text2)",display:"flex",alignItems:"center",gap:8}}>
                    <Info size={14} color={g.color}/> Save <strong style={{color:g.color,margin:"0 2px"}}>{fmt(monthly)}/month</strong> to reach on time
                  </div>
                )}
                <div style={{display:"flex",gap:8}}>
                  <input className="ft-input" type="number" placeholder="Add amount (press Enter)…" style={{flex:1}}
                    onKeyDown={e=>{if(e.key==="Enter"){const amt=parseFloat(e.target.value);if(!amt)return;const u=[...lg];u[i]={...g,saved:Math.min(g.saved+amt,g.target)};setLg(u);e.target.value="";showToast(`Added ${fmt(amt)} to "${g.name}".`);}}}/>
                  <button className="btn-danger" onClick={()=>{const u=lg.filter(x=>(x._id||x.id)!==gid);setLg(u);setGoals(u);showToast("Goal removed.","info");}}><Trash2 size={13}/></button>
                </div>
              </div>
            );
          })}
          <div className="card" style={{padding:24,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:180,border:"2px dashed var(--border2)",boxShadow:"none",cursor:"pointer",background:"var(--bg)",transition:"border-color .2s,background .2s"}}
            ...
            <div style={{width:46,height:46,borderRadius:12,background:"var(--blue-bg)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:12}}><Plus size={22} color="var(--blue)"/></div>
  ...
  const PageAnalytics=()=>{
    const monthlyData=useMemo(()=>{
      const map={};
      txns.forEach(t=>{const mo=t.date&&t.date.slice(0,7);if(!mo)return;if(!map[mo])map[mo]={income:0,expense:0};if(t.type==="credit")map[mo].income+=t.amount;else map[mo].expense+=t.amount;});
          ...
          {monthlyData.map(d=>(
            <div key={d.mo} className="card" style={{padding:22}}>
              ...
              <div style={{marginTop:14}}>
                ...
                <div className="progress-track"><div className="progress-fill" style={{width:`${clamp(d.savings/Math.max(d.income,1)*100,0,100)}%`,background:d.savings>0?"var(--blue)":"var(--red)"}}/></div>
                <div style={{fontSize:12,fontWeight:700,color:"var(--blue)",marginTop:4}}>{Math.round(d.savings/Math.max(d.income,1)*100)}%</div>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="card" style={{padding:22}}>
            ...
            {topPayees.map(([p,v],i)=>(
              <div key={p} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 0",borderBottom:"1px solid var(--border)"}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <div style={{width:24,height:24,borderRadius:7,background:"var(--bg)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:"var(--muted)"}}>#{i+1}</div>
            ...
            {catBreakdown.map(([cat,val])=>{
              ...
              return (
                <div key={cat} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 0",borderBottom:"1px solid var(--border)"}}>
                  <div style={{display:"flex",alignItems:"center",gap:9}}>{C?.Icon&&<C.Icon size={14} color={C.color}/>}<span style={{fontSize:13,color:"var(--text2)"}}>{C?.label}</span></div>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div className="progress-track" style={{width:80}}><div className="progress-fill" style={{width:`${catBreakdown[0]&&catBreakdown[0][1]>0?(val/catBreakdown[0][1])*100:0}%`,background:C?.color}}/></div>
        ...
        <div className="card" style={{padding:22}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
            {[
              ...
              {Icon:PiggyBank,title:"Savings rate",desc:`${Math.round(savings/Math.max(mtIncome,1)*100)}% — ${savings/Math.max(mtIncome,1)>0.2?"Above 20% target ✓":"Below 20% target"}`,bg:"#f0fdf4",bdr:"#bbf7d0",color:"#16a34a"},
              ...
              {Icon:CreditCard,title:"Average transaction",desc:`${fmt(mtExpense/Math.max(mtTxns.filter(t=>t.type==="debit").length,1),0)} per transaction`,bg:"#eff6ff",bdr:"#bfdbfe",color:"#2563eb"},
              {Icon:Target,title:"Best-progressed goal",desc:goals.length?`${goals.reduce((a,b)=>b.saved/b.target>a.saved/a.target?b:a,goals[0])?.name} (${Math.round(goals.reduce((a,b)=>b.saved/b.target>a.saved/a.target?b:a,goals[0])?.saved/Math.max(goals.reduce((a,b)=>b.saved/b.target>a.saved/a.target?b:a,goals[0])?.target,1)*100)}%)`:"No goals yet",bg:"#f0fdf4",bdr:"#bbf7d0",color:"#16a34a"},
            ].map((item,i)=>(
              <div key={i} className="insight-card" style={{background:item.bg,borderColor:item.bdr}}>
                <div style={{width:34,height:34,borderRadius:9,background:"white",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:10,boxShadow:"var(--shadow-xs)"}}><item.Icon size={16} color={item.color}/></div>
        ...
        <div className="modal-box card" style={{width:480,maxHeight:"90vh",overflowY:"auto",padding:0,boxShadow:"var(--shadow-lg)"}}>
          <div style={{padding:"18px 22px",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{fontSize:16,fontWeight:700,display:"flex",alignItems:"center",gap:8}}>
              {modal.type==="addTxn"?<><ArrowLeftRight size={16} color="var(--blue)"/>Add Transaction</>:<><Target size={16} color="var(--blue)"/>New Goal</>}
            </div>
            <button onClick={()=>setModal(null)} style={{background:"var(--bg)",border:"1px solid var(--border)",borderRadius:7,cursor:"pointer",width:30,height:30,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--muted)"}}><X size={15}/></button>
          </div>
          <div style={{padding:22}}>
                ...
                <div style={{display:"flex",gap:8,marginBottom:20,background:"var(--bg)",borderRadius:9,padding:4}}>
                  {["debit","credit"].map(t=>(
                    <button key={t} onClick={()=>setForm({...form,type:t})}
                      style={{flex:1,padding:"9px",borderRadius:7,cursor:"pointer",fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:13,fontWeight:600,transition:"all .15s",background:form.type===t?"white":"transparent",color:form.type===t?(t==="debit"?"var(--red)":"var(--green)"):"var(--muted)",border:"none",boxShadow:form.type===t?"var(--shadow-sm)":"none",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
            ...
            {modal.type==="addGoal"&&(
              <>
                {[["Goal Name","name","text","e.g. Emergency Fund"],["Target Amount (₹)","target","number","e.g. 100000"],["Already Saved (₹)","saved","number","0"],["Target Deadline","deadline","date",""]].map(([l,f,t,p])=>(
                  <div key={f} style={{marginBottom:14}}>
                    <label style={{display:"block",fontSize:12.5,fontWeight:600,color:"var(--text2)",marginBottom:6}}>{l}</label>
                ...
                <button className="btn-primary" style={{width:"100%",justifyContent:"center",padding:"11px",fontSize:14}}
                  onClick={()=>{if(!gf.name||!gf.target)return showToast("Goal name and target are required.","error");
                    const ng={_id:`g-${Date.now()}`,name:gf.name,target:parseFloat(gf.target),saved:parseFloat(gf.saved)||0,Icon:Target,deadline:gf.deadline||"2026-12-31",color:["#2563eb","#16a34a","#7c3aed","#d97706"][goals.length%4]};
  ...
  return (
    <>
      ...
      <div style={{display:"flex",minHeight:"100vh",background:"var(--bg)"}}>
        <aside style={{width:236,flexShrink:0,height:"100vh",position:"sticky",top:0,background:"var(--white)",borderRight:"1px solid var(--border)",display:"flex",flexDirection:"column",boxShadow:"var(--shadow-sm)"}}>
          <div style={{padding:"18px 20px 14px",borderBottom:"1px solid var(--border)"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <div style={{width:36,height:36,borderRadius:9,background:"var(--blue)",display:"flex",alignItems:"center",justifyContent:"center"}}><Wallet size={18} color="white"/></div>
              <div>
          ...
          <div style={{padding:"12px 16px",borderTop:"1px solid var(--border)"}}>
            <button className="btn-primary" style={{width:"100%",justifyContent:"center",marginBottom:12}} onClick={()=>setModal({type:"addTxn"})}><Plus size={15}/>Add Transaction</button>
            <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:9,background:"var(--bg)",border:"1px solid var(--border)"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:11,color:"var(--muted)"}}>{user.plan==="premium"?"Premium Plan":"Free Plan"}</div>
              </div>
              <button style={{background:"none",border:"none",cursor:"pointer",color:"var(--muted)",display:"flex",padding:2}} title="Sign out"
                onClick={()=>{localStorage.removeItem("fv_token");onLogout();}}>
                <LogOut size={14}/>
              </button>
            </div>
          </div>
      ...
      <Modal/>
      {toast&&(
        <div style={{position:"fixed",bottom:24,left:"50%",transform:"translateX(-50%)",background:toast.type==="error"?"var(--red)":toast.type==="info"?"#1f2937":"var(--green)",color:"white",padding:"11px 20px",borderRadius:10,fontWeight:600,fontSize:13,zIndex:9999,boxShadow:"var(--shadow-lg)",animation:"tslide .3s ease",display:"flex",alignItems:"center",gap:8,whiteSpace:"nowrap"}}>
